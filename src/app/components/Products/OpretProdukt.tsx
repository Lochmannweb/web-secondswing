...
export default function CreateProduct() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        Number.isNaN(productPayload.price) ||
  ...
  return (
    <Box className="opret-page">
      <h1 className="opret-page-title">Opret annonce</h1>

      <Box component="form" onSubmit={handleSubmit} className="opret-form">
        <Box className="opret-image-column">
          <Box className="opret-image-stage">
            <Box
              className="opret-image-track"
              sx={{ transform: `translateX(-${activePreviewIndex * 100}%)` }}
            >
              {(imagePreviews.length > 0 ? imagePreviews : ["/darkimgplaceholder.jpg"]).map(
                (imageSrc, index) => (
          ...
          <Button variant="outlined" component="label" fullWidth className="opret-image-button">
            {imagePreviews.length > 0 ? "Skift billeder" : "Upload billeder"}
